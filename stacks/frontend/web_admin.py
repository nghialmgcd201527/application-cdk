...
class WebAdminStack(Stack):
    """Class to create Web Admin"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        bucket_name = f"{project_name}-web-admin-{environment}"
        web_domain = conf.get("web_domain")
        web_admin_domain = conf.get("web_admin_domain")
        account_id = conf.get("account_id")

        # logs_bucket_name = f"{project_name}-{environment}-s3-access-logs"
        ...
        #     )
        # else:
        #     print(f"Logs Bucket {logs_bucket_name} already exists")

        my_hosted_zone = r53.HostedZone.from_lookup(
            self, f"{web_admin_domain}", domain_name=f"{web_domain}"
        )
        ...
        # web_site_bucket.node.add_dependency(logs_bucket)
        ...
        core.CfnOutput(
            ...
            export_name=f"{project_name}-cfn-web-admin-url",
        )
        ...
        core.CfnOutput(
            ...
            export_name=f"{project_name}-cfn-web-admin-id",
        )
        ...
        core.CfnOutput(
            ...
            export_name=f"{project_name}-bucket-web-admin-arn",
        )
